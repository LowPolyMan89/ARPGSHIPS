    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float GetAngleDeg(float2 uv)
            {
                ...
                float a = atan2(p.y, p.x);  // radians
                return degrees(a);          // -180..180
            }

            fixed4 frag (v2f i) : SV_Target
            {
                // базовый спрайт (круглый белый, например)
                ...
                {
                    // сектор через -180/180: angle >= start ИЛИ angle <= end
                ...
                // --------- РАДИАЛЬНЫЙ РИМ / FRESNEL ---------
                float2 p = (i.uv - 0.5) * 2.0;
                float r = length(p);             // 0 в центре, ~1 на краю
                float rim = saturate(1.0 - r);
                rim = pow(rim, 3.0);             // усиление у края
                ...
                float glow = rim * 0.7 + noise * 0.3 + ripple;

                fixed3 col = _Color.rgb * (0.4 + 0.6 * charge) * glow;

                float alpha = main.a * glow * charge;
